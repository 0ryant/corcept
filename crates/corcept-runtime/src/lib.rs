...
pub fn handle_hook(raw_json: &str, command: &str) -> Result<HookOutput> {
    // ADR-0006 v2 fast path — if the command is one of the 10 new canonical
    // hook names, route through the v2 dispatcher. The v2 payload is a
    // typed shape distinct from HookEnvelope, so we use
    // std::env::current_dir() (override via CORCEPT_ROOT). The v2 verdict
    // is translated to a legacy HookOutput shape for the CLI.
