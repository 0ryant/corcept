...
pub struct PrimerInjectionEvent {
    /// Wire-stable event kind. Stable for `priming_active` verifier matching.
    pub kind: &'static str,
...
impl BeforeRunHook for PrimerInjectionHook {
    fn run(&self, ctx: &mut BeforeRunContext) -> Result<HookOutcome, HookError> {
        ...
        let primer = match ctx.run_contract.primer.as_ref() {
            None => {
                let event = PrimerInjectionEvent {
                    kind: PrimerInjectionEvent::KIND,
                    primer_id: None,
                    primer_version: None,
                    primer_sha256: None,
        ...
        let mut new_prompt = String::with_capacity(body_str.len() + 1 + ctx.cell.system_prompt.len());
...
mod tests {
    ...
    /// primer + one trailing newline (the canonical body ends in '\n' so
    /// the trailing newline is reused — net growth is 1444 bytes).
    #[test]
