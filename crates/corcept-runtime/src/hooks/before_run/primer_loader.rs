...
pub enum PrimerLoaderError {
    ...
    /// The skill markdown was found but could not be parsed into a primer block.
...
impl PrimerLoader {
    ...
    fn try_read_live(
        ...
        let body = extract_priming_block(&raw).map_err(|reason| PrimerLoaderError::ExtractFailed {
            path: skill_path.clone(),
            reason,
        })?;
...
mod tests {
    ...
    fn env_override_takes_precedence_over_default() {
        ...
        // SAFETY: keep this test serial via the env-mutex below.
        let _g = ENV_LOCK.lock().unwrap_or_else(|p| p.into_inner());
        // Note: env::set_var is unsafe in Rust 2024+, but in 2021 (this
        // workspace's edition) it is safe. The lock above ensures no other
        // test races us.
    ...
    // Mutex to serialise tests that touch process env. std::env::set_var is
    // process-global; without this the env-override test would race with
    // future tests on the same key.
    use std::sync::Mutex;
