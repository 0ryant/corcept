//! Generated tool `corcept_hook_before_run`.
...
pub struct CorceptHookBeforeRunArgs {
...
impl McpTool for Tool {
    fn definition(&self) -> ToolDefinition {
        let schema = schemars::schema_for!(CorceptHookBeforeRunArgs);
        ToolDefinition {
            name: "corcept_hook_before_run".into(),
            title: Some("Corcept Hook: BeforeRun".into()),
            description: "ADR-0006 BeforeRun hook — fires when a run contract activates, before any tool use or model emission. Authority: Observe. Trust ceiling: Reviewed. Hook input JSON arrives on stdin (BeforeRunInput shape: run_contract, run_contract_fingerprint, trigger_type, trigger_payload_sha256, optional bindings).".into(),
    ...
    async fn call(&self, arguments: serde_json::Value) -> ToolCallResult {
        let args: CorceptHookBeforeRunArgs = match serde_json::from_value(arguments) {
            Ok(args) => args,
            Err(err) => return ToolCallResult::error(format!("invalid arguments: {err}")),
        };

        let tool_spec: mcpact_manifest::ToolSpec = match serde_json::from_str(include_str!(concat!("../../.mcpact/tools/corcept_hook_before_run.json"))) {
            Ok(spec) => spec,
        ...
        plan.argv.push("before-run".into());
        ...
        match Executor::default().execute(plan).await {
            Ok(result) => {
                let event = mcpact_audit::EvidenceEvent::tool_executed("corcept_hook_before_run", &plan_for_audit, &result);
                let sink = server_config::audit_sink();
                let _ = sink.emit(&event).await;
