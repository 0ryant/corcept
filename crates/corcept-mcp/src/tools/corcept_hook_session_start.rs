//! Generated tool `corcept_hook_session_start`.
...
pub struct CorceptHookSessionStartArgs {}
...
impl McpTool for Tool {
    fn definition(&self) -> ToolDefinition {
        let schema = schemars::schema_for!(CorceptHookSessionStartArgs);
        ToolDefinition {
            name: "corcept_hook_session_start".into(),
            title: Some("Corcept Hook: SessionStart".into()),
            description: "Process a Claude Code SessionStart hook event. Authority: Analyze. Trust ceiling: Reviewed. Hook input JSON arrives on stdin.".into(),
    ...
    async fn call(&self, arguments: serde_json::Value) -> ToolCallResult {
        let args: CorceptHookSessionStartArgs = match serde_json::from_value(arguments) {
            Ok(args) => args,
            Err(err) => return ToolCallResult::error(format!("invalid arguments: {err}")),
        };

        let tool_spec: mcpact_manifest::ToolSpec = match serde_json::from_str(include_str!(concat!("../../.mcpact/tools/corcept_hook_session_start.json"))) {
            Ok(spec) => spec,
        ...
        plan.argv.push("session-start".into());
        ...
        match Executor::default().execute(plan).await {
            Ok(result) => {
                let event = mcpact_audit::EvidenceEvent::tool_executed("corcept_hook_session_start", &plan_for_audit, &result);
                let sink = server_config::audit_sink();
                let _ = sink.emit(&event).await;
