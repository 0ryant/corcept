...
impl McpTool for Tool {
    fn definition(&self) -> ToolDefinition {
        ...
        ToolDefinition {
            ...
            description: "Process a Claude Code SessionStart hook event. Authority: Analyze. Trust ceiling: Reviewed. Pass the hook event JSON in the hook_input argument.".into(),
    ...
    async fn call(&self, arguments: serde_json::Value) -> ToolCallResult {
        ...
        let tool_spec: mcpact_manifest::ToolSpec = match serde_json::from_str(include_str!(
            concat!("../../.mcpact/tools/corcept_hook_session_start.json")
        )) {
            Ok(spec) => spec,
