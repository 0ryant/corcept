//! Generated tool `corcept_hook_on_claim_emitted`.
...
pub struct CorceptHookOnClaimEmittedArgs {
...
impl McpTool for Tool {
    fn definition(&self) -> ToolDefinition {
        let schema = schemars::schema_for!(CorceptHookOnClaimEmittedArgs);
        ToolDefinition {
            name: "corcept_hook_on_claim_emitted".into(),
            title: Some("Corcept Hook: OnClaimEmitted".into()),
            description: "ADR-0006 OnClaimEmitted hook — fires every time the model emits a typed claim (e.g. wrote_file, test_passed, evidence_emitted). The canonical extension point for tapprove.claim_audit. Authority: Observe. Trust ceiling: Reviewed. Hook input JSON: OnClaimEmittedInput (claim_shape, claim_sha256, optional target).".into(),
    ...
    async fn call(&self, arguments: serde_json::Value) -> ToolCallResult {
        let args: CorceptHookOnClaimEmittedArgs = match serde_json::from_value(arguments) {
            Ok(args) => args,
            Err(err) => return ToolCallResult::error(format!("invalid arguments: {err}")),
        };

        let tool_spec: mcpact_manifest::ToolSpec = match serde_json::from_str(include_str!(concat!("../../.mcpact/tools/corcept_hook_on_claim_emitted.json"))) {
            Ok(spec) => spec,
        ...
        plan.argv.push("on-claim-emitted".into());
        ...
        match Executor::default().execute(plan).await {
            Ok(result) => {
                let event = mcpact_audit::EvidenceEvent::tool_executed("corcept_hook_on_claim_emitted", &plan_for_audit, &result);
                let sink = server_config::audit_sink();
                let _ = sink.emit(&event).await;
