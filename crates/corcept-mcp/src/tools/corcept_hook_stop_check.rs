...
impl McpTool for Tool {
    ...
    async fn call(&self, arguments: serde_json::Value) -> ToolCallResult {
        ...
        let tool_spec: mcpact_manifest::ToolSpec = match serde_json::from_str(include_str!(concat!("../../.mcpact/tools/corcept_hook_stop_check.json"))) {
            Ok(spec) => spec,
        ...
        match Executor::default().execute(plan).await {
            Ok(result) => {
                let event = mcpact_audit::EvidenceEvent::tool_executed("corcept_hook_stop_check", &plan_for_audit, &result);
                let sink = server_config::audit_sink();
                let _ = sink.emit(&event).await;
