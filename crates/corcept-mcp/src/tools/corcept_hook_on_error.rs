//! Generated tool `corcept_hook_on_error`.
...
pub struct CorceptHookOnErrorArgs {
...
impl McpTool for Tool {
    fn definition(&self) -> ToolDefinition {
        let schema = schemars::schema_for!(CorceptHookOnErrorArgs);
        ToolDefinition {
            name: "corcept_hook_on_error".into(),
            title: Some("Corcept Hook: OnError".into()),
            description: "ADR-0006 OnError hook — fires on any phase error (validator fail, hook deny, tool error, verifier fail). Authority: Observe. Trust ceiling: Reviewed. Hook input JSON: OnErrorInput (phase, error_kind, optional error_sha256).".into(),
    ...
    async fn call(&self, arguments: serde_json::Value) -> ToolCallResult {
        let args: CorceptHookOnErrorArgs = match serde_json::from_value(arguments) {
            Ok(args) => args,
            Err(err) => return ToolCallResult::error(format!("invalid arguments: {err}")),
        };

        let tool_spec: mcpact_manifest::ToolSpec = match serde_json::from_str(include_str!(concat!("../../.mcpact/tools/corcept_hook_on_error.json"))) {
            Ok(spec) => spec,
        ...
        match Executor::default().execute(plan).await {
            Ok(result) => {
                let event = mcpact_audit::EvidenceEvent::tool_executed("corcept_hook_on_error", &plan_for_audit, &result);
                let sink = server_config::audit_sink();
                let _ = sink.emit(&event).await;
