//! Generated tool `corcept_hook_before_subprocess_spawn`.
...
pub struct CorceptHookBeforeSubprocessSpawnArgs {
...
impl McpTool for Tool {
    fn definition(&self) -> ToolDefinition {
        let schema = schemars::schema_for!(CorceptHookBeforeSubprocessSpawnArgs);
        ToolDefinition {
            name: "corcept_hook_before_subprocess_spawn".into(),
            title: Some("Corcept Hook: BeforeSubprocessSpawn".into()),
            description: "ADR-0006 BeforeSubprocessSpawn hook — fires before a tool implementation invokes Command::new / execve. Authority: Observe. Trust ceiling: Reviewed. Hook input JSON: BeforeSubprocessSpawnInput (program, argv, env, cwd). cellos enforcement layers on top.".into(),
    ...
    async fn call(&self, arguments: serde_json::Value) -> ToolCallResult {
        let args: CorceptHookBeforeSubprocessSpawnArgs = match serde_json::from_value(arguments) {
            Ok(args) => args,
            Err(err) => return ToolCallResult::error(format!("invalid arguments: {err}")),
        };

        let tool_spec: mcpact_manifest::ToolSpec = match serde_json::from_str(include_str!(concat!("../../.mcpact/tools/corcept_hook_before_subprocess_spawn.json"))) {
            Ok(spec) => spec,
        ...
        plan.argv.push("before-subprocess-spawn".into());
        ...
        match Executor::default().execute(plan).await {
            Ok(result) => {
                let event = mcpact_audit::EvidenceEvent::tool_executed("corcept_hook_before_subprocess_spawn", &plan_for_audit, &result);
                let sink = server_config::audit_sink();
                let _ = sink.emit(&event).await;
