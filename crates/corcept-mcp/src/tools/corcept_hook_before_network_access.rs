//! Generated tool `corcept_hook_before_network_access`.
...
pub struct CorceptHookBeforeNetworkAccessArgs {
...
impl McpTool for Tool {
    fn definition(&self) -> ToolDefinition {
        let schema = schemars::schema_for!(CorceptHookBeforeNetworkAccessArgs);
        ToolDefinition {
            name: "corcept_hook_before_network_access".into(),
            title: Some("Corcept Hook: BeforeNetworkAccess".into()),
            description: "ADR-0006 BeforeNetworkAccess hook — fires before any outbound socket / HTTP call. Authority: Observe. Trust ceiling: Reviewed. Hook input JSON: BeforeNetworkAccessInput (host, port, protocol, method).".into(),
    ...
    async fn call(&self, arguments: serde_json::Value) -> ToolCallResult {
        let args: CorceptHookBeforeNetworkAccessArgs = match serde_json::from_value(arguments) {
            Ok(args) => args,
            Err(err) => return ToolCallResult::error(format!("invalid arguments: {err}")),
        };

        let tool_spec: mcpact_manifest::ToolSpec = match serde_json::from_str(include_str!(concat!("../../.mcpact/tools/corcept_hook_before_network_access.json"))) {
            Ok(spec) => spec,
        ...
        plan.argv.push("before-network-access".into());
        ...
        match Executor::default().execute(plan).await {
            Ok(result) => {
                let event = mcpact_audit::EvidenceEvent::tool_executed("corcept_hook_before_network_access", &plan_for_audit, &result);
                let sink = server_config::audit_sink();
                let _ = sink.emit(&event).await;
