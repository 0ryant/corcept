//! Generated tool `corcept_hook_after_subprocess_exit`.
...
pub struct CorceptHookAfterSubprocessExitArgs {
...
impl McpTool for Tool {
    fn definition(&self) -> ToolDefinition {
        let schema = schemars::schema_for!(CorceptHookAfterSubprocessExitArgs);
        ToolDefinition {
            name: "corcept_hook_after_subprocess_exit".into(),
            title: Some("Corcept Hook: AfterSubprocessExit".into()),
            description: "ADR-0006 AfterSubprocessExit hook — fires when a subprocess returns. Authority: Observe. Trust ceiling: Reviewed. Hook input JSON: AfterSubprocessExitInput (program, argv, exit_code, stdout/stderr sha256, latency_ms).".into(),
    ...
    async fn call(&self, arguments: serde_json::Value) -> ToolCallResult {
        let args: CorceptHookAfterSubprocessExitArgs = match serde_json::from_value(arguments) {
            Ok(args) => args,
            Err(err) => return ToolCallResult::error(format!("invalid arguments: {err}")),
        };

        let tool_spec: mcpact_manifest::ToolSpec = match serde_json::from_str(include_str!(concat!("../../.mcpact/tools/corcept_hook_after_subprocess_exit.json"))) {
            Ok(spec) => spec,
        ...
        plan.argv.push("after-subprocess-exit".into());
        ...
        match Executor::default().execute(plan).await {
            Ok(result) => {
                let event = mcpact_audit::EvidenceEvent::tool_executed("corcept_hook_after_subprocess_exit", &plan_for_audit, &result);
                let sink = server_config::audit_sink();
                let _ = sink.emit(&event).await;
