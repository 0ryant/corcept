//! Generated tool `corcept_hook_after_run`.
...
pub struct CorceptHookAfterRunArgs {
...
impl McpTool for Tool {
    fn definition(&self) -> ToolDefinition {
        let schema = schemars::schema_for!(CorceptHookAfterRunArgs);
        ToolDefinition {
            name: "corcept_hook_after_run".into(),
            title: Some("Corcept Hook: AfterRun".into()),
            description: "ADR-0006 AfterRun hook — fires when the run completes (any outcome). Authority: Observe. Trust ceiling: Reviewed. Hook input JSON: AfterRunInput (run_contract, run_contract_fingerprint, status, optional artefacts list).".into(),
    ...
    async fn call(&self, arguments: serde_json::Value) -> ToolCallResult {
        let args: CorceptHookAfterRunArgs = match serde_json::from_value(arguments) {
            Ok(args) => args,
            Err(err) => return ToolCallResult::error(format!("invalid arguments: {err}")),
        };

        let tool_spec: mcpact_manifest::ToolSpec = match serde_json::from_str(include_str!(concat!("../../.mcpact/tools/corcept_hook_after_run.json"))) {
            Ok(spec) => spec,
        ...
        match Executor::default().execute(plan).await {
            Ok(result) => {
                let event = mcpact_audit::EvidenceEvent::tool_executed("corcept_hook_after_run", &plan_for_audit, &result);
                let sink = server_config::audit_sink();
                let _ = sink.emit(&event).await;
