//! Generated tool `corcept_hook_user_prompt_submit`.
...
pub struct CorceptHookUserPromptSubmitArgs {
...
impl McpTool for Tool {
    fn definition(&self) -> ToolDefinition {
        let schema = schemars::schema_for!(CorceptHookUserPromptSubmitArgs);
        ToolDefinition {
            name: "corcept_hook_user_prompt_submit".into(),
            title: Some("Corcept Hook: UserPromptSubmit".into()),
            description: "Process a Claude Code UserPromptSubmit hook event. Authority: Analyze. Trust ceiling: Reviewed. Hook input JSON arrives on stdin.".into(),
            input_schema: serde_json::to_value(&schema).unwrap_or_else(|_| json!({"type":"object"})),
            output_schema: None,
            annotations: Some(mcpact_mcp::ToolDefinition::mcpact_annotations(mcpact_core::AuthorityClass::Analyze, server_config::TRUST)),
        }
    }

    async fn call(&self, arguments: serde_json::Value) -> ToolCallResult {
        let args: CorceptHookUserPromptSubmitArgs = match serde_json::from_value(arguments) {
            Ok(args) => args,
            Err(err) => return ToolCallResult::error(format!("invalid arguments: {err}")),
        };

        let tool_spec: mcpact_manifest::ToolSpec = match serde_json::from_str(include_str!(concat!("../../.mcpact/tools/corcept_hook_user_prompt_submit.json"))) {
            Ok(spec) => spec,
        ...
        match mcpact_policy::evaluate_invocation(&ctx, &tool_spec, &args_json) {
            ...
            Ok(decision) => {
                ...
                let event = mcpact_audit::EvidenceEvent::tool_denied(
                    ...
                    mcpact_core::AuthorityClass::Analyze,
                    &reason,
                );
        ...
        plan.argv.push("user-prompt-submit".into());
        ...
        plan.authority = mcpact_core::AuthorityClass::Analyze;

        let plan_for_audit = plan.clone();
        match Executor::default().execute(plan).await {
            Ok(result) => {
                let event = mcpact_audit::EvidenceEvent::tool_executed("corcept_hook_user_prompt_submit", &plan_for_audit, &result);
                let sink = server_config::audit_sink();
                let _ = sink.emit(&event).await;
