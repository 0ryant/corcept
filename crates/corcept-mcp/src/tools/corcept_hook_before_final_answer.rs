//! Generated tool `corcept_hook_before_final_answer`.
...
pub struct CorceptHookBeforeFinalAnswerArgs {
...
impl McpTool for Tool {
    fn definition(&self) -> ToolDefinition {
        let schema = schemars::schema_for!(CorceptHookBeforeFinalAnswerArgs);
        ToolDefinition {
            name: "corcept_hook_before_final_answer".into(),
            title: Some("Corcept Hook: BeforeFinalAnswer".into()),
            description: "ADR-0006 BeforeFinalAnswer hook — fires once before the model's terminal response is emitted. Closes the Tools-Haiku silent-stub class (FINAL-COMPOSITE.md:29). Authority: Observe. Trust ceiling: Reviewed. Hook input JSON: BeforeFinalAnswerInput (final_answer_sha256, final_answer_bytes, run_contract).".into(),
    ...
    async fn call(&self, arguments: serde_json::Value) -> ToolCallResult {
        let args: CorceptHookBeforeFinalAnswerArgs = match serde_json::from_value(arguments) {
            Ok(args) => args,
            Err(err) => return ToolCallResult::error(format!("invalid arguments: {err}")),
        };

        let tool_spec: mcpact_manifest::ToolSpec = match serde_json::from_str(include_str!(concat!("../../.mcpact/tools/corcept_hook_before_final_answer.json"))) {
            Ok(spec) => spec,
        ...
        plan.argv.push("before-final-answer".into());
        ...
        match Executor::default().execute(plan).await {
            Ok(result) => {
                let event = mcpact_audit::EvidenceEvent::tool_executed("corcept_hook_before_final_answer", &plan_for_audit, &result);
                let sink = server_config::audit_sink();
                let _ = sink.emit(&event).await;
