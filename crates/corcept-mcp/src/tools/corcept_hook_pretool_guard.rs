...
impl McpTool for Tool {
    fn definition(&self) -> ToolDefinition {
        ...
        ToolDefinition {
            ...
            description: "Evaluate the PreToolUse guard against a candidate tool call. Authority: Plan (returns a gate decision without itself mutating the ledger; the matched posttool-audit row attests the actual decision downstream). Trust ceiling: Reviewed. Pass the hook event JSON in the hook_input argument.".into(),
    ...
    async fn call(&self, arguments: serde_json::Value) -> ToolCallResult {
        ...
        let tool_spec: mcpact_manifest::ToolSpec = match serde_json::from_str(include_str!(
            concat!("../../.mcpact/tools/corcept_hook_pretool_guard.json")
        )) {
            Ok(spec) => spec,
