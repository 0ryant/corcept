...
impl McpTool for Tool {
    fn definition(&self) -> ToolDefinition {
        let schema = schemars::schema_for!(CorceptDoctorArgs);
        ToolDefinition {
            name: "corcept_doctor".into(),
            title: Some("Corcept Doctor".into()),
            description: "Run corcept's health-check (ledger presence, permissions, schema sanity). Authority: Observe. Trust ceiling: Reviewed.".into(),
    ...
    async fn call(&self, arguments: serde_json::Value) -> ToolCallResult {
        let args: CorceptDoctorArgs = match serde_json::from_value(arguments) {
            Ok(args) => args,
            Err(err) => return ToolCallResult::error(format!("invalid arguments: {err}")),
        };

        let tool_spec: mcpact_manifest::ToolSpec = match serde_json::from_str(include_str!(concat!("../../.mcpact/tools/corcept_doctor.json"))) {
            Ok(spec) => spec,
        ...
        plan.argv = Vec::new();
        plan.argv.push("doctor".into());
        if args.path.is_some() {
        plan.argv.push("--path".into());
        plan.argv.push(match args.path { Some(v) => v, None => String::new() });
        }
        if args.strict { plan.argv.push("--strict".into()); }
        if args.validate_perms { plan.argv.push("--validate-perms".into()); }
        ...
        match Executor::default().execute(plan).await {
            Ok(result) => {
                let event = mcpact_audit::EvidenceEvent::tool_executed("corcept_doctor", &plan_for_audit, &result);
                let sink = server_config::audit_sink();
                let _ = sink.emit(&event).await;
